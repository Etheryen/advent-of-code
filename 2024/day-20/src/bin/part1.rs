...
impl Coordinates {
    ...
    fn passed_through(&self, obstacles: &HashSet<Coordinates>) -> Vec<Coordinates> {
        let all = [
            // Up
            Coordinates {
                x: self.x,
...
fn time_saving_cheats(
    ...
    for (coords_before, steps_before) in &normal_path {
        for coords_after in coords_before.passed_through(&obstacles) {
            let least_steps = normal_path.get(&coords_after).unwrap();
            let steps_after = steps_before + 2;
...
// const SIDE: i32 = 15;
// const CHEAT_SAVED_STEP_MIN: u32 = 1;
const SIDE: i32 = 141;
