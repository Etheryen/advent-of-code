...
fn to_antinodes(antennas: &HashMap<char, HashSet<Coordinates>>, map: &Map) -> HashSet<Coordinates> {
    ...
    for locations in antennas.values() {
        for (coords1, coords2) in to_pairs(locations) {
            antinodes.insert(coords1.clone());
            antinodes.insert(coords2.clone());

            let new_antinodes = to_new_antinodes(coords1, coords2, map);

            for new_antinote in new_antinodes {
                if is_in_map(&new_antinote, map) {
                    antinodes.insert(new_antinote);
                }
            }
        }
    }
